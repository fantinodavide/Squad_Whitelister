...
<template>
	<popup ref="popupLogin" title="Edit List" @cancelBtnClick="$emit('cancelBtnClick', $event)" @confirmBtnClick="confirmBtnClick" :extRetProp="extRet">
		...
		<SelectMultiple :elements="discord_roles" oIdKey="id" oTitleKey="name" title="Discord Roles" :preselect="data.list_obj.discord_roles" @selectChanged="extRet.discord_roles = $event" />
